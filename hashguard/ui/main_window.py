...
from hashguard.config import (
    get_current_theme_colors, load_config, save_config, adjust_color, THEMES, SUCCESS, WARNING, VERSION
)
...
class HashGuard(TkinterDnD.Tk):
    ...
    def _build_header(self):
        # Premium, highly polished GHP-style topbar header!
        self.hdr = ctk.CTkFrame(self, fg_color=self._get_color_for_role("BG_CARD"), corner_radius=0, height=72)
        ...
        # Brand Icon (Locks symbol with glow styling)
        icon_lbl = ctk.CTkLabel(
            self.hdr,
            text="🔒",
            font=("Segoe UI", 24),
        )
        icon_lbl.theme_role = "accent"
        icon_lbl.grid(row=0, column=0, sticky="w", padx=(28, 12), pady=12)

        # Title & Subtitle Info Container
        brand_info = ctk.CTkFrame(self.hdr, fg_color="transparent")
        brand_info.grid(row=0, column=1, sticky="w", pady=12)
        ...
        sub_lbl = ctk.CTkLabel(
            brand_info,
            text=f"v{VERSION}  •  Desktop Hashing Hub",
            font=("Segoe UI", 12),
        )
        sub_lbl.theme_role = "dim"
        sub_lbl.pack(anchor="w")

        # Sleek Settings button on the right (looks exactly like GHP header tab actions)
        ...
        settings_btn.grid(row=0, column=2, sticky="e", padx=28, pady=18)
    ...
    def _build_body(self):
        ...
        sect1.grid(row=0, column=0, sticky="w", padx=padx, pady=(15, 2))

        methods_card = self._card(body)
        methods_card.grid(row=1, column=0, sticky="ew", padx=padx, pady=(0, 10))
        ...
        desc_lbl.grid(row=0, column=0, columnspan=4, sticky="w", padx=18, pady=(14, 10))
        ...
        for algo, info in HASH_ALGORITHMS.items():
            ...
            cb.grid(row=1, column=col, padx=(18 if col == 0 else 14, 14), pady=(0, 16), sticky="w")
            col += 1

        sect2 = self._section_label(body, "SELECT OR DRAG & DROP FILE")
        sect2.grid(row=2, column=0, sticky="w", padx=padx, pady=(12, 2))

        file_card = self._card(body)
        file_card.grid(row=3, column=0, sticky="ew", padx=padx, pady=(0, 10))
        ...
        self.file_entry.grid(row=0, column=0, sticky="ew", padx=(18, 12), pady=(16, 12))
        ...
        browse_btn.grid(row=0, column=1, padx=(0, 18), pady=(16, 12))
        ...
        self.drop_zone.grid(row=1, column=0, columnspan=2, sticky="ew", padx=18, pady=(0, 16))
        ...
        self.progress.set(0)
        self.progress.grid(row=4, column=0, sticky="ew", padx=padx, pady=(6, 0))
        ...
        sect3.grid(row=6, column=0, sticky="w", padx=padx, pady=(12, 2))

        hash_card = self._card(body)
        hash_card.grid(row=7, column=0, sticky="ew", padx=padx, pady=(0, 10))
        hash_card.grid_columnconfigure(0, weight=1)

        self.hash_rows_frame = ctk.CTkFrame(hash_card, fg_color="transparent")
        self.hash_rows_frame.grid(row=0, column=0, sticky="ew", padx=18, pady=(16, 6))
        ...
        actions.grid(row=1, column=0, padx=18, pady=(6, 16), sticky="ew")
        ...
        compute_btn.grid(row=0, column=0, padx=(0, 8), sticky="ew")
        ...
        export_btn.grid(row=0, column=1, padx=(8, 0), sticky="ew")

        sect4 = self._section_label(body, "VERIFY INTEGRITY")
        sect4.grid(row=8, column=0, sticky="w", padx=padx, pady=(12, 2))

        verify_card = self._card(body)
        verify_card.grid(row=9, column=0, sticky="ew", padx=padx, pady=(0, 14))
        ...
        desc_lbl2.grid(row=0, column=0, columnspan=4, sticky="w", padx=18, pady=(16, 10))
        ...
        self.verify_entry.grid(row=1, column=0, sticky="ew", padx=(18, 12), pady=(0, 12))
        ...
        paste_btn.grid(row=1, column=1, padx=(0, 8), pady=(0, 12))
        ...
        load_btn.grid(row=1, column=2, padx=(0, 8), pady=(0, 12))
        ...
        verify_btn.grid(row=1, column=3, padx=(0, 18), pady=(0, 12))

        self.detected_label = ctk.CTkLabel(verify_card, text="", font=("Segoe UI", 12))
        self.detected_label.theme_role = "dim"
        self.detected_label.grid(row=2, column=0, columnspan=4, sticky="w", padx=18, pady=(0, 10))
        ...
        self.hash_file_entry.grid(row=3, column=0, columnspan=4, sticky="ew", padx=18, pady=(0, 12))
        ...
        self.hash_drop_zone.grid(row=4, column=0, columnspan=4, sticky="ew", padx=18, pady=(0, 12))
        ...
        self.loaded_hashes_label.grid(row=5, column=0, columnspan=4, sticky="w", padx=18, pady=(0, 10))
        self.loaded_hashes_label.grid_remove()

        self.result_frame = ctk.CTkFrame(verify_card, fg_color="transparent", corner_radius=10)
        self.result_frame.grid(row=6, column=0, columnspan=4, sticky="ew", padx=18, pady=(0, 16))
        ...
        self.result_label.grid(row=0, column=0, padx=18, pady=12, sticky="w")
        ...
        footer_lbl.grid(row=10, column=0, pady=(16, 20))
    ...
    def _card(self, parent):
        # 14px rounded corners and subtle outlines perfectly matching the cards in GHP
    ...
    def _create_hash_row(self, parent, row, label, var, copy_cmd):
        ...
        lbl.theme_role = "dim"
        lbl.grid(row=row, column=0, padx=(0, 12), pady=(0, 10))
        ...
        ent.grid(row=row, column=1, sticky="ew", pady=(0, 10))
        ...
        btn.grid(row=row, column=2, padx=(12, 0), pady=(0, 10))
        return lbl, ent, btn
    ...
    def apply_theme(self):
        ...
        # Standard tk.Tk uses 'bg' configure
        self.configure(bg=self._theme_colors["BG_ROOT"])

...
def apply_theme_to_widget(widget, theme):
    ...
    if isinstance(widget, ctk.CTkFrame):
        if role == "root":
            ...
            widget.configure(fg_color=adjust_color(bg_card, -0.15), border_color=adjust_color(bg_card, 0.2))
        else:
            widget.configure(fg_color=bg_card)
        ...
        if role == "accent":
            ...
            widget.configure(fg_color=accent, hover_color=adjust_color(accent, -0.15), text_color=txt_col)
        elif role == "neutral":
            neutral_bg = adjust_color(bg_card, 0.15)
            widget.configure(fg_color=neutral_bg, hover_color=adjust_color(neutral_bg, 0.15), text_color=fg_main)
        else:
            txt_col = "#FFFFFF" if is_dark(accent) else "#000000"
            widget.configure(fg_color=accent, hover_color=adjust_color(accent, -0.15), text_color=txt_col)
        ...
        widget.configure(
            fg_color=adjust_color(bg_card, -0.1), 
        ...
        widget.configure(
            fg_color=accent, 
            hover_color=adjust_color(accent, -0.15), 
            text_color=fg_main, 
            border_color=adjust_color(bg_card, 0.2)
        )
        
    elif isinstance(widget, ctk.CTkProgressBar):
        widget.configure(progress_color=accent, fg_color=adjust_color(bg_card, -0.1))
